...
import { Bot, Save, RefreshCw, Sparkles, User, MessageSquare, Briefcase } from "lucide-react"
...
export default function AsistentePage() {
  ...
  // Config fields (from clients table)
  ...
  // custom_context: additional info the assistant should always know
  // Stored in client_preferences as "assistant_context"
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {TONES.map((t) => (
            ...
            >
              <p className={`font-medium text-sm ${tone === t.code ? "text-[#06B6D4]" : "text-[#0F1F63]"}`}>
                {t.label}
              </p>
      ...
      <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {STYLES.map((s) => (
            ...
            >
              <p className={`font-medium text-sm ${style === s.code ? "text-[#3B82F6]" : "text-[#0F1F63]"}`}>
                {s.label}
              </p>
      ...
      <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Información que Operaly siempre debe saber sobre ti o tu trabajo. Por ejemplo: tu empresa, tus clientes principales, tus objetivos del mes, herramientas que usas.
        </p>
        <textarea
          value={customContext}
      ...
      <div className="flex items-center gap-3">
        ...
        {saved && (
          <p className="text-sm text-[#34D399] font-medium">✓ Guardado correctamente</p>
        )}
      </div>
    </div>
  )
