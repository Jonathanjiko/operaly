...
// ── Phone validator for ANY country ──────────────────────────────────────────
function validateAnyPhone(input: string, countryCode: string): { ok: boolean; value: string; error: string } {
  ...
  // If starts with + → international format, validate length only
  ...
  // No + prefix — need country dial code
  ...
  if (!dial) {
    // Unknown country — if 7-15 digits, try as-is with +
...
export default function SetupClient() {
  ...
  const nextStep = () => {
    if (step === 4) {
      ...
      if (!result.ok) { setPhoneError(result.error); return }
      setPhoneError("")
    }
  ...
  const handleFinish = async () => {
    ...
    if (!normalized.ok) { alert(normalized.error); return }
    ...
    try {
      ...
      // ── Step 3: Write client_id to auth user metadata ──────────────────
      // This writes to user_metadata. The DB trigger copies it to app_metadata.
      const { error: updateUserError } = await supabase.auth.updateUser({
        data: {
          ...
          client_id:          clientId,   // ← CRITICAL: backend reads this
      ...
      // ── Step 5: Queue welcome WhatsApp message ─────────────────────────
      await supabase.rpc("queue_welcome_message", { p_client_id: clientId })

      // ── Step 6: Refresh session so new metadata is available ───────────
      const { data: refreshData } = await supabase.auth.refreshSession()
      const finalClientId = refreshData?.session?.user?.user_metadata?.client_id || clientId
      ...
      localStorage.setItem("operaly_assistant_profile", JSON.stringify({
        fullName, profession, countryCode, city,
        phone_normalized: normalized.value,
        preferred_language: language,
        timezone: browserTimeZone,
        planCode,
      ...
      // ── Step 8: Route ───────────────────────────────────────────────────
      if (planCode === "trial") {
        // Trial: go directly to dashboard
        router.push("/dashboard")
      } else {
        // Paid plan: go to payment first
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {ASSISTANT_PROFESSIONS.map(item => (
                  <button key={item.code} type="button" onClick={() => setProfession(item.code)}
